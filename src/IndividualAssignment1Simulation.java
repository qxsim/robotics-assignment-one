...
public class IndividualAssignment1Simulation {
	...
	public void run() {
		...
		// simulated robot.
		Pose startingPose = new Pose(3.0f, 3.0f, 0);
		...
		OctagonController controller2 = new OctagonController(wrapper.getRobot(), 0.5f);
		NonagonController controller3 = new NonagonController(wrapper.getRobot(), 0.5f);
		BumperController controller4 = new BumperController(wrapper.getRobot());
		...
		sim.addTouchSensorListener(wrapper, controller4);
		...
		//controller3.run();
		controller4.run();
		controller4.setRangeScanner(ranger);
